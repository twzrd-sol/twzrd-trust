...
| **Clearance SKU** | Paid Execution | Signed `twzrd.payment_decision.v1` / `quickCheck` at **$0.001** (`GET /v1/intel/quick/{payTo}`). Primary paid product. Offline verify: `npx twzrd-payment-decision --verify`. |
...
| **Path A / V7 Receipt** | Paid Intelligence | Optional $0.05 USDC detailed reputation intelligence and portable signed Ed25519 V7 receipt (`GET /v1/intel/trust/{pubkey}`). **Not the primary SKU.** Offline-verifiable via `twzrd-receipt-verifier@^1.4.0` (V5/V6/V7). |
