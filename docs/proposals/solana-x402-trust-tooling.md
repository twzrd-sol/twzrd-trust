...
**Construction, not the universe.** There is no x402 program on Solana. A corpus row is an SPL Token transfer of classic USDC (`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`) whose fee payer (first signer) is in the published known-facilitator list (`x402-solana-facilitators.json`, compiled into the ingest allow-list). Payer = owner of the source token account; merchant = owner of the dest token account. Helius enhanced webhook + reconciler operate **inside that list**. Self-facilitated (payer or merchant pays own gas), unknown third-party sponsors, merchants who never used a known facilitator, and non-USDC mints are out of scope. A 2026-07-10 100-merchant / 72h sample found unknown-sponsor plus self-facilitated dollar volume comparable to the known-facilitator slice; those classes remain unindexed. LaserStream pattern-detect is a 2026-07-05 proposal, not running. Honest claim: **known-facilitator classic USDC, watermarked** — a live subset, not Michelin of every stall.
