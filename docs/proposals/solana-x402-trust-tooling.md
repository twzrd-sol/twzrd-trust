...
| Buyer hook | `twzrd-x402-gate@0.8.19` (docs-only republish of the `0.8.18` compile; `dist/` and `bin/` byte-identical) |
...
Package registry metadata is the release source of truth. The public mirror was brought byte-identical to the published artifact, and the gate's TypeScript source, tests, and examples were published, on 2026-08-19 — before this application; none of that synchronization is funded work. Registry `latest` is `0.8.19`, a documentation-only republish of the same compiled artifact (`dist/` and `bin/` byte-identical to `0.8.18`; only the manifest and install docs changed). Live hosted pins (`llms.txt`, `skill.md`, `.well-known/agent.json`) were updated to `0.8.19` on 2026-08-19 and name that same compile. Known manifest defect, disclosed rather than hidden: the published manifest's `live-autogate-matrix` script names an example file that was never committed; removing or implementing it is Milestone 1 work. Milestone 1 funds what remains: deterministic fixtures, the compatibility matrix, transcript-schema reconciliation, manifest cleanup, and CI that fails on drift.

### Dated Solana corpus baseline

**Construction, not the universe.** There is no x402 program on Solana. A corpus row is an SPL Token transfer of classic USDC (`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`) whose fee payer (first signer) is in the published known-facilitator list (`x402-solana-facilitators.json`, compiled into the ingest allow-list). Payer = owner of the source token account; merchant = owner of the dest token account. Helius enhanced webhook + reconciler operate **inside that list**. Self-facilitated (payer or merchant pays own gas), unknown third-party sponsors, merchants who never used a known facilitator, and non-USDC mints are out of scope. A 2026-07-10 100-merchant / 72h sample found unknown-sponsor plus self-facilitated dollar volume comparable to the known-facilitator slice; those classes remain unindexed. LaserStream pattern-detect is a 2026-07-05 proposal, not running. Honest claim: **all Solana x402 through known facilitators, classic USDC** — a live subset, not Michelin of every stall.

Public dashboard values from the dated refresh of **that index**, vintage **2026-08-18 UTC** (Dune dashboard 215276; freshness query 7913460 execution `01M0CPK69Y83EKFMYJ6AJF0RAW` on 2026-08-19 reports `kpi_vintage_day=2026-08-18`, `corpus_last_payment=2026-08-18 23:59:45+00`, `corpus_freshness=FRESH`). Definitions, the reproduction query, and exclusion arithmetic are in the [current corpus note](./corpus-note-2026-08-18.md). Prior dated refreshes: [`2026-08-16`](./corpus-note-2026-08-16.md), [`2026-07-09`](./corpus-note-2026-07-09.md).
