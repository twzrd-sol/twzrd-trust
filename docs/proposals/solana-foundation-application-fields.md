...
Primary rail: Solana. MIT-licensed public gate: twzrd-x402-gate (buyer hook — TypeScript source, tests, and the published package in the public mirror), hosted MCP at intel.twzrd.xyz, skill at intel.twzrd.xyz/skill.md. Settlement graph is observed payment behavior, not a service catalog.
...
- Deterministic local fixtures + productized proof harness (block and clean-control legs), documented @x402/core + x402-solana@2.1.0 integrations with a tested compatibility matrix. The mirror-version synchronization completed 2026-08-19 is not funded work.
