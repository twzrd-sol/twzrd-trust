...
const VERDICT_CODES = { allow: 1, warn: 2, block: 3 };
const OUTCOME_CODES = {
    settled: 1,
...
function keccak256(data) {
    ...
    try {
        ...
        throw new Error("[twzrd] outcome attestations need js-sha3 (keccak256 — same dep as " +
...
function base58() {
    ...
    try {
        ...
        throw new Error("[twzrd] outcome attestations need @scure/base (base58). Install it with: npm i @scure/base");
    }
    return mod.base58;
}
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
function boundedUtf8(value, maxBytes, field) {
    const raw = Buffer.from(value ?? "", "utf8");
    if (raw.length === 0)
        throw new Error(`[twzrd] ${field} must be non-empty`);
...
function intentHash32(intentHash) {
    if (intentHash === null || intentHash === undefined)
        return null;
...
function anchor32(settlementTx) {
    if (!settlementTx)
        return null;
...
function checkContract(verdict, outcome, hasAnchor, hasPayer) {
    if (!(verdict in VERDICT_CODES))
        throw new Error(`[twzrd] unknown verdict ${verdict}`);
    if (!(outcome in OUTCOME_CODES))
        throw new Error(`[twzrd] unknown outcome ${outcome}`);
    if (outcome === "settled") {
        if (!hasAnchor)
            throw new Error("[twzrd] outcome=settled requires a settlement anchor");
        if (!hasPayer)
            throw new Error("[twzrd] outcome=settled requires a payer");
        if (verdict === "block") {
...
export function computeDecisionOutcomeLeafV1(fields) {
    const { decisionId, counterparty, verdict, outcome, timestampUnix } = fields;
    checkContract(verdict, outcome, Boolean(fields.settlementTx), fields.payer !== null && fields.payer !== undefined);
...
function rawPubkeyFromPem(publicKeyPem) {
 ...
 */
export async function buildOutcomeAttestation(fields, signer) {
    const leaf = computeDecisionOutcomeLeafV1(fields);
    const b58 = base58();
    const signature = Buffer.from(await signer.sign(leaf));
    if (signature.length !== 64) {
        throw new Error(`[twzrd] decision signer produced a ${signature.length}-byte signature (expected 64)`);
    }
 ...
 */
export async function buildBlockedNeverSignedAttestation(token, options) {
    if (token.decision !== "block") {
        throw new Error(`[twzrd] blocked_never_signed attestation requires a BLOCK token (got ${token.decision})`);
    }
 ...
 */
export function verifyOutcomeAttestationSignature(attestation, expectedPubkeyB58) {
    const leafHex = (attestation.leaf || "").toLowerCase().replace(/^0x/, "");
    if (!/^[0-9a-f]{64}$/.test(leafHex) || !attestation.signature)
        return false;
    try {
        const b58 = base58();
        ...
        return edVerify(null, Buffer.from(leafHex, "hex"), createPublicKey({ key: spki, format: "der", type: "spki" }), sig);
    }
    catch {
        return false;
    }
}
//# sourceMappingURL=outcome-attestation.js.map