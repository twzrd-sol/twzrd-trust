...
const agent = new SolanaAgentKit(wallet, rpcUrl, {
  beforeSign: twzrdBeforeSign({ refuseWashFlagged: true }),
        ...
        note: "Requires the optional beforeSign hook on Config (sendaifun/solana-agent-kit#599). " +
...
export function detectHosts(pkg) {
    if (!pkg)
        return [];
    const all = {
        ...(pkg.dependencies ?? {}),
 ...
 */
export async function proveRefusal() {
    ...
    try {
        ...
        if (out && typeof out === "object" && out.abort) {
            refused = true;
            reason = String(out.reason ?? "abort");
        }
    }
    catch (err) {
