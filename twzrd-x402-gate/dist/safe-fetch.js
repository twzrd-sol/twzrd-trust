...
function emit(opts, order, event, detail) {
    order.push(event);
 ...
 */
export async function safeFetch(opts) {
    const eventOrder = [];
...
export async function main(argv = process.argv.slice(2)) {
    ...
    const result = await safeFetch({
        ...
        unsupportedNetworkMode: getFlag("--unsupported-network-mode"),
    ...
    if (jsonOut) {
        ...
        console.log(typeof result.resourceBody === "string"
            ? result.resourceBody
            : JSON.stringify(result.resourceBody, null, 2));
    }
    else if (result.phase === "dry_run_allowed") {
        console.error(`[twzrd-safe-fetch] dry-run allowed payTo=${result.payTo} reason=${result.approval?.reason}`);
    }
    else if (!result.ok) {
        console.error(`[twzrd-safe-fetch] ${result.phase}: ${result.error ?? result.approval?.reason ?? "failed"}`);
    }
    if (result.phase === "blocked")
        return 2;
    if (result.phase === "payer_failed" || result.phase === "error")
        return 1;
    if (result.phase === "passthrough" && !result.ok)
        return 1;
    return 0;
}
