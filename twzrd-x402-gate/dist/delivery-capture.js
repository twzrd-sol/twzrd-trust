...
export function extractAdvertisedOutput(challengeBody) {
    const none = { jsonExpected: false, exampleKeys: [] };
    ...
    if (!output)
        return none;
    const jsonExpected = typeof output.type === "string" && output.type.toLowerCase() === "json";
 ...
 */
export function assessSchemaMatch(resourceBody, advertised) {
    const bodyObj = asRecord(resourceBody);
    if (advertised.exampleKeys.length > 0) {
        const schemaMatch = bodyObj != null && advertised.exampleKeys.every((k) => k in bodyObj);
    ...
    const nonEmpty = resourceBody != null &&
 ...
 */
export function extractSettlementTx(payerOutput, depth = 0) {
    const rec = asRecord(payerOutput);
    if (!rec || depth > 2)
        return null;
    for (const [key, value] of Object.entries(rec)) {
        if (TX_KEYS.has(key.toLowerCase()) &&
            typeof value === "string" &&
 ...
 */
export async function captureDeliveryObservation(args, cfg) {
...
//# sourceMappingURL=delivery-capture.js.map