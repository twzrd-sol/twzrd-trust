...
async function run() {
  ...
  // String() so TS compares runtime values rather than narrowing both sides to
  // string literals and declaring the comparison unintentional (TS2367). The
  // comparison is the whole proof: the scored requirement vs what the payer saw.
  const secureWouldBlock =
    String(r.requirementScored?.payTo) === String(payerSawPayTo) &&
    String(r.requirementScored?.amountMicro) === String(payerSawAmount);
  assert.equal(
    secureWouldBlock,
    false,
