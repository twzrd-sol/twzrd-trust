...
/** The sponsor backend that actually settles the x402 challenge. */
export type SponsorSettle = (url: string, init?: RequestInit) => Promise<Response>;

export interface SponsoredX402Options {
  /**
   * The funded backend that settles on the agent's behalf — the agent's @x402/svm
   * fetch (gas-sponsored) or a treasury sponsor-endpoint fetch (full-sponsor).
   * Omit (with dryRun) for a no-spend mock.
   */
  settle?: SponsorSettle;
  /** No real settle — returns `dryRunBody` as a 200. Default false. */
  dryRun?: boolean;
  /** Telemetry/audit: called with each settled URL (wire your per-agent budget cap here). */
  onSettle?: (url: string) => void;
  /** Body returned in dryRun (default `{ sponsored_dry_run: true }`). */
  dryRunBody?: unknown;
}
 ...
 */
export function createSponsoredX402Fetch(opts: SponsoredX402Options = {}): typeof fetch {
  ...
  return (async (input: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]) => {
    ...
    return settle(url, init as RequestInit | undefined);
