 ...
 */
function microUnitsToDecimal(units: string, decimals: number): string {
  const t = units.trim();
  if (!/^\d+$/.test(t) || decimals <= 0) return t;
...
export function x402RequirementsToIntent(
...
export type Ap2Cart = {
  ...
  currency: string;
  total: string;
  items?: Array<{ sku?: string; description?: string; price?: string }>;
  checkoutUrl?: string;
  category?: string;
  recurring?: boolean;
  priorCharge?: string;
...
export function ap2CheckoutToIntent(
  cart: Ap2Cart,
  mandate: Ap2UserMandate,
  options?: { network?: string; agentId?: string; organization?: string },
): { intent: PaymentIntent; mandate: Mandate } {
  ...
  const normalizedMandate: Mandate = {
    mandateId: mandate.mandateId,
