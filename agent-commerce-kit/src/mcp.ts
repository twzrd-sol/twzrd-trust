...
for await (const line of process.stdin) {
  try {
    ...
    else if (request.method === "tools/call" && request.params?.name === "agent_commerce_simulate") { const run = simulate(request.params.arguments); result = { content: [{ type: "text", text: JSON.stringify(exportEvidence(run.loop)) }] }; }
    else if (request.method === "tools/call" && request.params?.name === "agent_commerce_replay") result = { content: [{ type: "text", text: JSON.stringify(replay(request.params.arguments.events)) }] };
    else throw new Error("method_or_tool_not_found");
    console.log(JSON.stringify({ jsonrpc: "2.0", id: request.id, result }));
  } catch (error) { console.log(JSON.stringify({ jsonrpc: "2.0", id: null, error: { code: -32603, message: error instanceof Error ? error.message : String(error) } })); }
}
