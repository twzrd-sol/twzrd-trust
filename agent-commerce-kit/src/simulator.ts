...
export interface SimulationOptions { input: CreateLoopInput; decision: Decision; paymentSucceeds?: boolean; deliverySucceeds?: boolean; }
export function simulate(options: SimulationOptions): { loop: AgentCommerceLoop; signerInvocations: number } {
  const loop = createLoop(options.input); let signerInvocations = 0;
  applyPolicy(loop, options.decision, [`simulated_${options.decision}`], `preflight:${loop.loop_id}`);
  if (options.decision === "block") return { loop, signerInvocations };
  createPaymentIntent(loop); signerInvocations += 1; markPaymentSigned(loop, `sig:sim:${loop.loop_id}`);
  if (options.paymentSucceeds === false) { markPaymentFailed(loop, "simulated_payment_failure"); return { loop, signerInvocations }; }
